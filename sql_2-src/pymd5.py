#!/usr/bin/python3
# originally based from this python 2 gist : https://gist.github.com/HoLyVieR/11e464a91b290e33b38e
# Derived from:
#
...
def padding(msg_bits):
    ...
    if msg_bits % 8:
        raise ValueError("msg_bits must be an integer divisible by 8")

    index = (msg_bits >> 3) % md5.block_size
    index += 8 # the last 8 bytes store the number of bits in the message

    if index < md5.block_size:
        padLen = md5.block_size - index
    else:
        padLen = 2 * md5.block_size - index
